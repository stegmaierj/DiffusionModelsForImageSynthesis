...
class DiffusionModel2D(pl.LightningModule):
    ...
    def __init__(self, hparams):
        ...
        if self.hparams.backbone.lower() == 'unet2d_pixelshuffle_inject':
            from models.module_UNet2D_pixelshuffle_inject import module_UNet2D_pixelshuffle_inject as backbone
        else:
    ...
    def train_dataloader(self):
         if self.hparams.train_list is None:
            ...
            dataset = MeristemH5Dataset(self.hparams.train_list, self.hparams.data_root, patch_size=self.hparams.patch_size,\
                                        image_groups=self.hparams.image_groups, mask_groups=self.hparams.mask_groups, reduce_dim=True,\
    ...
    def test_dataloader(self):
        if self.hparams.test_list is None:
            ...
            dataset = MeristemH5Dataset(self.hparams.test_list, self.hparams.data_root, patch_size=self.hparams.patch_size, reduce_dim=True,\
    ...
    def val_dataloader(self):
        if self.hparams.val_list is None:
            ...
            dataset = MeristemH5Dataset(self.hparams.val_list, self.hparams.data_root, patch_size=self.hparams.patch_size, reduce_dim=True,\
                                        image_groups=self.hparams.image_groups, mask_groups=self.hparams.mask_groups, augmentation_dict={},\
                                        data_norm=self.hparams.data_norm, no_mask=True, boundary_handling='none',\
                                        image_noise_channel=self.hparams.image_noise_channel, mask_noise_channel=self.hparams.mask_noise_channel, noise_type=self.hparams.noise_type)
            return DataLoader(dataset, batch_size=self.hparams.batch_size)
    ...
    def add_model_specific_args(parent_parser): 
        ...
        # network params
        parser.add_argument('--backbone', default='UNet2D_PixelShuffle_inject', type=str, help='which model to load (UNet3D_PixelShuffle_inject)')
        ...
        parser.add_argument('--patch_size', default=(1,256,256), type=int, nargs='+')
        ...
        parser.add_argument('--data_root', default='/netshares/BiomedicalImageAnalysis', type=str) 
        parser.add_argument('--train_list', default='/work/scratch/eschweiler/filelists/CTC_N2DL-HeLa_simmask.csv', type=str)
        parser.add_argument('--test_list', default='/work/scratch/eschweiler/filelists/CTC_N2DL-HeLa_simmask.csv', type=str)
        parser.add_argument('--val_list', default='/work/scratch/eschweiler/filelists/CTC_N2DL-HeLa_simmask.csv', type=str)
        parser.add_argument('--image_groups', default=('data/diffusion_mask',), type=str, nargs='+')
        parser.add_argument('--mask_groups', default=('data/boundary',), type=str, nargs='+')
